...
define( 'INSIDE', true );
	...
	class GPage extends VillagePage {
		...
		function handleRallyPoint() {
			...
			$troops_in_village = 'troops_in_village';
			$troopsTable = 'troopsTable';
			$troops_num = 'troops_num';
			$troopsIntrapTable = 'troopsIntrapTable';
			$troops_intrap_num = 'troops_intrap_num';
			$troops_out_village = 'troops_out_village';
			$troops_out_num = 'troops_out_num';
			$troops_out_intrap_num = 'troops_out_intrap_num';
			$troops_in_oases = 'troops_in_oases';
			$war_to_village = 'war_to_village';
			$war_from_village = 'war_from_village';
			$war_to_oasis = 'war_to_oasis';
			
			$this->rallyPointProperty = array( $troops_in_village => array( $troopsTable => $this->_getTroopsList( $troops_num ),
			$troopsIntrapTable => $this->_getTroopsList( $troops_intrap_num ) ),
			$troops_out_village => array( $troopsTable => $this->_getTroopsList( $troops_out_num ),
		...
		function handleResidencePalace() {
			...
			if ($this->selectedTabIndex == 0) {
				...
				if ($this->selectedTabIndex == 1) {
					...
					while ($result->next(  )) {
						list( , $cpRate ) = explode( ' ', $result->row['cp'] );
						list( $this->cpValue ) = $cpRate;
