...
class WarBattleModel extends BattleModel
{
    function handleWarAttack( $taskRow, $toVillageRow, $fromVillageRow, $procInfo )
    {
        ...
        foreach ( $bStrArr as $key => $value ) {
            list( $item_id, $level, $update_state ) = explode( ' ', $value );
            if ( $item_id == 27 ) {
                $_hasTresury = true;
            }
        }
        $checkToVillageRow        = $this->_getVillageInfo( $taskRow['village_id'] );
        $villageTotallyDestructed = TRUE;
        $bStr                     = trim( $checkToVillageRow['buildings'] );
        if ( $bStr != '' ) {
            $bStrArr = explode( ',', $bStr );
        ...
        if ( !$_hasTresury ) {
            if ( $_hasOwnTresury ) {
                if ( $procInfo['troopsArray']['hasHero'] ) {
                    $checkq = mysql_query( "SELECT * FROM v_artefacts WHERE vref=" . $taskRow['to_village_id'] . "" );
                    if ( mysql_num_rows( $checkq ) > 0 ) {
                        $taskRow['proc_stealartefact'] = 1;
                    }
                }
            }
        }
        if ( $warResult['all_defense_killed'] == 1 ) {
            if ( $taskRow['proc_stealartefact'] == 1 ) {
                $checkoursartefact = mysql_query( "SELECT * FROM v_artefacts WHERE owner='" . $taskRow['player_id'] . "'" );
                $artcount          = mysql_num_rows( $checkoursartefact );
                if ( $artcount > 2 ) {
                    $taskRow['proc_stealartefact'] = 2;
                } else {
                    while ( $toCheck = mysql_fetch_assoc( $checkq ) ) {
                        if ( $artcount < 3 ) {
                            mysql_query( "UPDATE v_artefacts SET vref='" . $taskRow['village_id'] . "',conquered='" . time() . "',owner='" . $taskRow['player_id'] . "' WHERE id='" . $toCheck['id'] . "'" );
                            $artcount++;
                        }
                    }
                }
