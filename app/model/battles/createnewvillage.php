...
/**
*
* @   Script Name :   createnewvillage.php
* @   Author      :   NIKO28
* @   Skype       :   nicolo3767
...
class NewVillageBattleModel extends BattleModel {
	function handleCreateNewVillage($taskRow, $toVillageRow, $fromVillageRow, $cropConsumption) {
		...
		$this->provider->executeQuery( 'UPDATE p_villages v
			...
			WHERE v.id=%s', array( $cropConsumption, $child_villages_id, intval( $fromVillageRow['id'] ) ) );
...
';
		...
		$this->provider->executeQuery( 'UPDATE p_players p
			SET
				...
				p.create_nvil=1
			WHERE
				p.id=%s', array( intval( $toVillageRow['id'] ), $villages_id, $villages_data, intval( $fromVillageRow['player_id'] ) ) );
		return FALSE;
	}
}
