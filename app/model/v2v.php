...
class WarModel extends ModelBase {
	function getPlayerDataById($playerId) {
		...
		return $this->provider->fetchRow( 'SELECT
			...
			FROM p_players p WHERE p.id=%s', array( $protectionPeriod, $playerId ) );
	}
	...
	function backTroopsFrom($fromVillageId, $column1, $troops1, $toVillageId, $column2, $troops2) {
	...
	function hasNewVillageTask($playerId) {
