...
class VillageModel extends ModelBase {
	function getLatestReports($playerId, $villageId) {
		return $this->provider->fetchResultSet( 'SELECT
			...
			LIMIT 5', array( $playerId, $villageId ) );
	}

	function getLatestReports2($fromPlayerIds, $playerId, $villageId) {
		return $this->provider->fetchResultSet( 'SELECT
				r.id,
				r.rpt_result,
				DATE_FORMAT(r.creation_date, \'%%y/%%m/%%d\') mdate,
			...
			ORDER BY r.creation_date DESC
			LIMIT 5', array( $playerId, $villageId, $fromPlayerIds ) );
	}

	function getAlliancePlayersId($alliance_id) {
	...
	function getMapItemData($villageId) {
