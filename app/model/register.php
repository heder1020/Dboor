...
class RegisterModel extends ModelBase {
	...
	function isPlayerEmailExists($playerEmail) {
		return 0 < $this->provider->fetchScalar( 'SELECT COUNT(*) FROM p_players p WHERE p.email=\'%s\'', array( $playerEmail ) );
	}

	function _getEmptyVillageId($position, $mapSize) {
		$halfMapSize = floor( $mapSize / 2 );
		$positionArray = array( 0 - $halfMapSize, $halfMapSize, 0 - $halfMapSize, $halfMapSize );
		switch ($position) {
		case 1: {
				$positionArray = array( 0 - $halfMapSize, 0, 0, $halfMapSize );
				break;
			}

		case 2: {
				$positionArray = array( 0, $halfMapSize, 0, $halfMapSize );
				break;
			}

		case 3: {
				$positionArray = array( 0 - $halfMapSize, 0, 0 - $halfMapSize, 0 );
				break;
			}

		case 4: {
				$positionArray = array( 0, $halfMapSize, 0 - $halfMapSize, 0 );
			}
		}
	...
	function createVillage($playerId, $tribeId, $villageId, $playerName, $villageName, $playerType) {
		...
		$this->provider->executeQuery( 'UPDATE p_villages v
			SET
				...
				v.troops_num=\'%s\',
				v.update_key=\'%s\'
			WHERE v.id=%s', array( $tribeId, $playerId, $playerName, $villageName, ($isSpecial ? '1' : '0'), $buildings, $resources, $troops_training, $troops_num, $update_key, $villageId ) );
		return TRUE;
	}
...
';
		...
		$this->provider->executeQuery( 'UPDATE p_players
			SET
				...
				villages_count=%s
			WHERE
				id=%s', array( $activationCode, $villageId, $villages_id, $villages_data, sizeof( $villages ), $playerId ) );
		$expr = '';
		switch ($tribeId) {
		case 1: {
