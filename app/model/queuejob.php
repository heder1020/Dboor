...
class QueueJobModel extends ModelBase
{
    ...
    function cropBalance( $playerId, $villageId )
    {
        ...
        if ( $row == NULL )
            return null;
        $playerRow = $this->provider->fetchRow( "SELECT p.villages_data FROM p_players p WHERE p.id=%s", Array(
             $playerId 
        ) );
        ...
        $resArr = $this->_getResourcesArray( $row[ 'resources' ], $row[ 'elapsedTimeInSeconds' ], $row[ 'crop_consumption' ], $row[ 'cp' ] );
        $crop   = $resArr[ 'resources' ][ 4 ][ 'current_value' ];
        if ( $crop < 0 ) {
            $tArr = $this->_getTroopsArray( $row[ 'troops_num' ] );
            arsort( $tArr );
            foreach ( $tArr as $k => $v )
            ...
            foreach ( $tArr as $k => $v )
                ksort( $tArr[ $k ] );
            $troops                                        = $this->_getTroopsString( $tArr );
            $cp                                            = $resArr[ 'cp' ][ 'cpValue' ] . " " . $resArr[ 'cp' ][ 'cpRate' ];
            $resArr[ 'resources' ][ 4 ][ 'current_value' ] = $crop;
            $resources                                     = $this->_getResourcesString( $resArr[ 'resources' ] );
