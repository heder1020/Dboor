...
class QueueJobModel extends ModelBase {
	...
	function deletePlayer($playerId) {
		...
		if (0 < intval( $row['alliance_id'] )) {
			...
			$_aRow = $this->provider->fetchRow( 'SELECT a.players_ids, a.player_count FROM p_alliances a WHERE a.id=%s', array( intval( $row['alliance_id'] ) ) );
	...
	function captureOasis($oasisId, $playerId, $villageId, $capture = TRUE) {
		...
		if ($capture) {
			$this->provider->executeQuery( 'UPDATE p_villages v
				...
				WHERE v.id=%s', array( intval( $villageId ), intval( $villageRow['tribe_id'] ), intval( $villageRow['player_id'] ), (0 < intval( $villageRow['alliance_id'] ) ? intval( $villageRow['alliance_id'] ) : 'NULL'), $villageRow['player_name'], $villageRow['alliance_name'], intval( $oasisId ) ) );
		}
		else {
			$this->provider->executeQuery( 'UPDATE p_villages v
				SET
	...
	function upgradeBuilding($villageId, $bid, $itemId, $drop = FALSE) {
		...
		case 40: {
				if ($newBuildingLevel == sizeof( $buildingMetadata['levels'] )) {
					...
					QueueTask;
					( new $queueModel->addTask( QS_SITE_RESET, 0, $resetTime ) );
