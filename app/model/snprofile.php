...
class SNprofileModel extends ModelBase {
	function SendNews($post) {
		$this->provider->executeQuery( 'INSERT INTO `p_profile` SET
			...
			`youtube` = \'%s\',
			`date` = \'%s\'
			 ;', array( $post['userid'], $post['message'], $post['image'], $post['url'], $post['youtube'], time(  ) ) );
	}

	function SendComment($post) {
		$this->provider->executeQuery( 'INSERT INTO `p_comment` SET
			`userid` = \'%s\',
			`username` = \'%s\',
			`to_userid` = \'%s\',
			`topicid` = \'%s\',
			`comment` = \'%s\',
			`date` = \'%s\'
			 ;', array( $post['userid'], $post['username'], $post['to_userid'], $post['topicid'], $post['comment'], time(  ) ) );
	}

	function DeleteNews($newsID, $playerId) {
		$this->provider->executeQuery( 'DELETE FROM `p_profile` WHERE `ID` = \'%s\' AND `userid` = \'%s\' ;', array( $newsID, $playerId ) );
	}

	function DeleteComment($commentID, $playerId) {
	...
	function GetNews($playerId, $pageIndex, $pageSize) {
	...
	function getNewsCount($playerId) {
	...
	function getFriendData($playerId) {
