...
class BadWordsModel extends ModelBase
{
    function addBadWords( $BadWords )
    {
        ...
        while ( $i < count( $BadWords ) ) {
            $this->provider->executeQuery( 'INSERT INTO `g_words` SET `word` = \'%s\' ;', array(
                 $BadWords[$i] 
            ) );
            ++$i;
        }
    ...
    {
        $this->provider->executeQuery( 'DELETE FROM `g_words` WHERE `ID` = \'%s\' ;', array(
             $BadWordID 
        ) );
    }
    function GetBadWords( $pageIndex, $pageSize )
    {
        return $this->provider->fetchResultSet( 'SELECT * FROM `g_words` LIMIT %s,%s;;', array(
             $pageIndex * $pageSize,
            $pageSize 
        ) );
    }
